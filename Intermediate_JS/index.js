...
const returnedp=thisisnow("as","Ateef",1000,"red")
.then(()=>{
    thisisnow("HEADING2","sohail",1000,"blue")
})
.catch(()=>{
    thisisnow("HEADING3","Tousif",1000,"blue")
})
.then(()=>{
    console.log("this is the 2nd then")
})
.finally(()=>{
    console.log("this is final")
})

const url = "http://localhost:3000/posts"; // Ensure this URL is correct
...
json to normal object we can conver it throuhg the parse


/fetch-----------------it should come with 2 then that is first response.json and then displaying json or something
...
---fetch post
import fetch from 'node-fetch'; // only if you need fetch in a Node.js-like environment
...
fetch('http://localhost:3000/posts', {
    ...
    headers: {
        'Content-Type': "application/json; charset=utf-8", // Ensure correct header format
    },
})
.then((response) => {
    if (!response.ok) {
